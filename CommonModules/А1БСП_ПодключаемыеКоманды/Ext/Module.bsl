...
#Если НЕ Клиент Тогда
	...
	Функция ФормаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
		...
		Если А1Э_Формы.ТипФормы(Форма) = "ФормаСписка" Тогда
			ОсновнойСписок = А1Э_Формы.ОсновнойСписок(Форма); 
			Если ОсновнойСписок <> Неопределено Тогда
				А1Э_УниверсальнаяФорма.ДобавитьОбработчикЭлементаФормы(Форма, ОсновнойСписок, "ПриАктивизацииСтроки", "А1БСП_ПодключаемыеКоманды.ПриАктивизацииСтроки");
			КонецЕсли;	
		КонецЕсли;
...
Функция ОбъектИлиТаблицаФормы(Форма)
	...
	ИначеЕсли А1Э_Формы.ТипФормы(Форма) = "ФормаСписка" Тогда
		Возврат А1Э_Формы.ОсновнойСписок(Форма);
	Иначе
...
#Если НЕ Клиент Тогда
	...
	Функция ПриОпределенииОбъектовСКомандамиПечати(СписокОбъектов) Экспорт 
		ОбъектыМеханизма = А1Э_Механизмы.ОбъектыМеханизма("А1БСП_ПодключаемыеКоманды");
